...
## 3bis. 与 vLLM 报告同口径的 prefill / decode / 投机解码 A/B（2026-09-14）

脚本：`scripts/bench_llama.py`（单次测试）+ `scripts/bench_ab.sh`（自动对比 baseline vs ngram-mod）
原始数据：`data/bench_20260914_{baseline,ngram_mod}.json`

防坑口径（同 `research/vllm/qwen35-4b-awq/` 的两条教训）：
* 每次请求带随机 nonce + `cache_prompt:false`；表里回显 `cache_n`，**实测全程 0（无缓存命中）**
* prompt 长度用 `/tokenize` + `/detokenize` 精确构造（并扣除 chat 模板固定开销），误差 ±1 token
* 3 次取最优；GPU 峰值显存 9216–9222 MiB、峰值功耗 250 W（两次配置一致，说明投机解码零显存代价）

### prefill（max_tokens=1，输入 128/512/2048/4096/8000）

| 目标长度 | 引擎 prompt_n | TTFT（引擎） | TTFT（wall） | 吞吐 |
|---|---|---|---|---|
| 128 | 127 | 0.397 s | 0.658 s | 320 tok/s |
| 512 | 511 | 0.878 s | 1.288 s | 582 tok/s |
| 2048 | 2047 | 2.812 s | 3.262 s | 728 tok/s |
| 4096 | 4095 | 5.426 s | 5.871 s | 755 tok/s |
| 8000 | 7999 | 10.623 s | 11.093 s | 753 tok/s |

* 短 prompt 的 TTFT 被固定开销主导（128 token 的 wall TTFT 0.66 s 里约 0.3 s 是引擎/调度固定开销），≥4096 后吞吐稳定在 **~750 tok/s**。
* 投机解码对 prefill **无影响**（747 vs 753 tok/s，0.99×）。

### decode

| 输出长度 | baseline TPOT / 速度 | ngram-mod TPOT / 速度 | 倍率 |
|---|---|---|---|
| 128 | 28.26 ms → 35.4 tok/s | 28.51 ms → 35.1 tok/s | 0.99× |
| 512 | 28.69 ms → 34.9 tok/s | **13.84 ms → 72.3 tok/s** | **2.07×** |

> 输出越长，模型自身的重复片段越多，n-gram 草稿越容易命中 → 收益随长度上升（自由生成也能到 2×）。

### 投机解码三类任务（输出 256 token）

| 任务 | baseline | ngram-mod | 倍率 | 草稿接受率 |
|---|---|---|---|---|
| 逐字复述（原样重复输入） | 35.3 tok/s | **93.2 tok/s** | 2.64× | 60.0%（201/335） |
| **代码改写**（变量改名，输出≈输入） | 35.2 tok/s | **254.8 tok/s** | **7.23×** | **100%（245/245）** |
| 开放问答（自由生成） | 35.2 tok/s | 53.5 tok/s | 1.52× | 54.7%（140/256） |

**结论**：投机解码对"输出与输入高度重叠"的任务（改代码、抄录、diff 式编辑）收益 2.6–7.2×，
对自由生成也有 1.5–2×（取决于输出长度）；对 prefill 零影响、显存零代价 → **建议常开**。
